...
function assertPath(userId, providerName, providerId, extension) {
  console.log("Create path", path.join(destPath, getPath(userId, providerName)))
...
function getPathAndFile(userId, providerName, providerId, extension) {
  return `${getPath(userId, providerName, providerId, extension)}/${getFilename(providerId, extension)}`
}

function getFullPathAndFile(userId, providerName, providerId, extension) {
  return path.join(destPath, `${getPath(userId, providerName, providerId, extension)}/${getFilename(providerId, extension)}`)
}

function getUrl(userId, providerName, providerId, extension) {
  return `http://localhost:3010/${getPathAndFile(userId, providerName, providerId, extension)}`
}
