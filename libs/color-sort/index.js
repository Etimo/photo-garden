...
const sort = (photos) => {
  ...
  for (let i = 0; i < photos.length; i++) {
    for (let j = 0; j < i; j++) {
      distances.push([photos[i], photos[j], colorDistance(photos[i].color, photos[j].color)]);
    }
  }
  ...
  for (let i = 0; i < distances.length; i++) {
    ...
    if (photo1 != cluster1[cluster1.length - 1])
      cluster1.reverse();
    if (photo2 != cluster2[0])
      cluster2.reverse();
...
const colorDistance = (color1, color2) => Math.sqrt(2 * Math.pow(color2.r - color1.r, 2) + 4 * Math.pow(color2.g - color1.g, 2) + 3 * Math.pow(color2.b - color1.b, 2));
