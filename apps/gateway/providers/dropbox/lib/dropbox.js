...
function getFrom(token, path, request, queryparams) {
    return new Promise((resolve, reject) => {
        const req = https.request(getPostOptions(path, token, request, queryparams), res => {
        ...
        if (request) { req.write(JSON.stringify(request)); }
        else { req.write(); }
...
const getThumbnails = (token, photoIds) => {
    ...
    return success(getFrom(token, '2/files/get_thumbnail_batch', {}, {
            data: {
                entries: entries
            }
        }));
...
module.exports = {
    getPhotos: (token, nextPageToken) => {
        return success(getFrom(token, '/2/files/search', {
            path: '',
            query: '.jpg',
            start: nextPageToken,
    ...
    getUserInfo: (token, accountId) => {
        return success(getFrom(token, '/2/users/get_account', {
